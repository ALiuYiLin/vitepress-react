...
要创建团队页面，首先，创建一个新的 mdx 文件。文件名无所谓，这里我们就叫它 `team.mdx` 吧。在这个文件中，在 frontmatter 设置 `layout: page`，然后你可以使用 `TeamPage` 组件来组成页面结构。
...
为此，将 `<VPTeamPageSection>` 组件添加到我们之前创建的 `team.mdx` 文件中。
