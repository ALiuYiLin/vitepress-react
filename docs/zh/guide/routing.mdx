...
可以使用参数将附加数据传递到每个页面。本 fork 是 React 语义（没有 `$params` 全局或 `{{ }}` 插值，规则见[在 Markdown 中使用 React](./using-react)）：`params` 由当前路由提供，[`useData()`](../reference/runtime-api#usedata) 是 React hook，只能在组件（含自定义/主题组件）内调用；页面正文想直接展示动态数据时，把读取封装成组件即可：
...
`useData()` 不能在模块顶层调用（hook 只能在组件体内执行）；`params` 即当前路由的参数对象。
