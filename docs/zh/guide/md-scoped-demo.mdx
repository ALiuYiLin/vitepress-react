...
description: 在 Markdown 中使用 Vue-like 页面级 scoped 样式 —— 外部 *.scoped.css 导入
---
...
| 写法 | 说明 |
| --- | --- |
| 内联 `<style scoped>…</style>` | 仅旧 markdown-it 内核(md 页)支持;mdx 文档页不支持(见下方「用法一」说明) |
| 外部 `*.scoped.css` 导入 | 在文档顶层直接 `import './xx.scoped.css'`(模块级导入),样式放独立文件 |
...
直接在 md 正文里写 `<style scoped>…</style>` 是旧 markdown-it 内核(md 页)支持的
写法;在 .mdx 文档页(新 mdx 内核)中不再支持:mdx 会把 `<style>` 当作 JSX 标签
解析,块内 CSS 的 `{}` 不是合法的 JSX 子文本,整页会编译直接失败;新内核也不
提取正文里的 style 块。页面级 scoped 样式请统一使用下面的
「用法二:外部 `*.scoped.css` 导入」;全局样式可写到站点样式或主题组件中。

```md
<!-- 反例:.mdx 正文中不要直接写 <style scoped> 块(CSS 的 {} 会令编译失败) -->
  <style scoped>
  .example { color: red; }
  </style>
```
...
/ `.scoped.less` 结尾),再在文档顶层按**相对本 .mdx 文件**的路径 import:
...
下方卡片用的就是 `./md-scoped-demo.scoped.css`(本页顶部已真实 import;
类名由卡片段**之后**的独立 attrs 行 `\{.md-scoped-card\}` 注入——mdx 内核的
attrs 书写为 `\{…\}`,作用于紧邻其前的块):
...
- **hash 唯一**:`data-v-{hash}` 由页面文件绝对路径生成,每一页各不相同;
  同页多个 scoped 文件共用同一 hash;
- **页面内所有 DOM 都会被注入 `data-v-{hash}`**(编译期由管线统一注入),
  因此选择器也可以直接按元素写,例如 `.vp-doc p`(只会命中本页段落);
- **给正文元素加类**(mdx 内核 attrs,书写为 `\{…\}`,作用于紧邻其前的块):标题 `## x \{#id\}` 行尾(固定锚点)、块级元素后独立一行 `\{.class\}`、链接后 `[文字](url)\{target="_blank"\}` 等;若在正文里直接写整行 JSX(React 接管),
  则按 JSX 规则用 `className`;
- **全局样式**仍写不带 `scoped` 的 `<style>`(运行时注入全站);mdx 文档页不支持
  正文内联 `<style scoped>`,多个外部 scoped 文件可同时出现在一页;
