...
- **块级**:单独成行(连续无空行的多行也可),整段作为一个完整 fragment,如 `<><Foo /> {count}</>`。

约束:标签必须严格 `<>`…`</>`(漏斜杠 / 带空格即退回字面文本);内部是 JSX children,属性按 JSX 写(`className`、驼峰事件;Vue 指令不接管);fragment 必须可配平,任意复杂跨多行 JSX 仍用 [`::: react` 容器](#react-container)。
:::
