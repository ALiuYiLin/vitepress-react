...
块级 `<>…</>` 已支持跨多行(含空行),常规"一整块 JSX"用它写即可。`::: react` 容器保留给块级 fragment **取代不了**的场景:内容**不以 `<>` 开头**、**不需要 `<>…</>` 配平**(如含不成对尖括号/裸 `</>` 的教学文本)、**不含 `{` 或 `<` 的纯文本 JSX 区**,或想用**显式 `:::` 行收尾**,以及**缩进上下文**(列表/引用内;fragment 要求列 0)。容器只认首行 `::: react` 与收尾 `:::` 两行,中间内容对 markdown-it 完全不透明、原样交给 React:
