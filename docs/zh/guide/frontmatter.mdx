...
本 fork 的 Markdown 是 React 语义（见[在 Markdown 中使用 React](./using-react)），没有 Vue 的 `$frontmatter` 全局与 `{{ }}` 插值。mdx 内核会把页面的 YAML frontmatter 自动导出为模块绑定 `frontmatter`，正文里直接 `{frontmatter.xxx}` 引用即可：
