...
VitePress 使用 React 的服务端渲染能力，在生产构建期间于 Node.js 中预渲染整个应用。这意味着主题组件、mdx 页面模块里的组件以及任何自定义代码都要考虑 **SSR 兼容性**。

[React 官方 SSR 文档](https://react.dev/reference/react-dom/server) 介绍了 SSR / SSG 是什么以及如何编写服务端/客户端都能运行的代码。原则非常简单：**只在 effect（`useEffect`/`useLayoutEffect`）里访问浏览器或 DOM API**——服务端预渲染不会执行 effect，因此写在 effect 内的副作用天然 SSR 安全；组件体内只放 hooks 调用与纯计算。
