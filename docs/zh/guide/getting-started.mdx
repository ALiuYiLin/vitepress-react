...
<Snippet src="@/snippets/init.ansi" />

::: tip React 作为依赖
vitepress 自身已内置 react/react-dom 供默认主题与 md 页面运行时使用；如果你要在自定义主题或页面模块里直接 `import 'react'`，建议在项目中显式安装 `react` 与 `react-dom`（版本以 vitepress 依赖的 React 19 为准）。
:::
