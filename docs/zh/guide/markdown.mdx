...
想在 Markdown 里写组件、`{expr}` 表达式或页面级 scoped 样式？请先读[在 Markdown 中使用 React](./using-react)(表达式、组件、样式规则都在其中)与[md 页面 scoped 样式](./md-scoped-demo)。本页其余语法（标题锚点、容器、代码高亮等）与上游一致。
:::
...
这允许将标题链接为 `#my-anchor`，而不是默认的 `#使用自定义锚点`(mdx 内核下 attrs 书写为 `\{…\}`,标题文本不会显示残字)。
...
可以用 `<Snippet src="…" />` 从现有文件导入代码片段——它是编译期识别的标签,不需要 `import`:

```md
<Snippet src="@/filepath" />
```

此语法同时支持[行高亮](#line-highlighting-in-code-blocks)：

```md
<Snippet src="@/filepath" lines="…" />
```

**输入**

```md
<Snippet src="@/snippets/snippet.js" lines="2" />
```
...
也可以使用 [VS Code region](https://code.visualstudio.com/docs/editor/codebasics#_folding) 只包含代码文件的相应部分:用 `region` 属性提供自定义区域名:

**输入**

```md
<Snippet src="@/snippets/snippet-with-region.js" region="snippet" lines="1" />
```

**Code file**

<Snippet src="@/snippets/snippet-with-region.js" />
...
```md
<Snippet src="@/snippets/snippet.cs" lang="c#" />

<!-- 带行高亮: -->

<Snippet src="@/snippets/snippet.cs" lines="1,2,4-6" lang="c#" />

<!-- 带行号: -->

<Snippet src="@/snippets/snippet.cs" lines="1,2,4-6" lang="c#" attrs=":line-numbers" />
```
...
<Snippet src="@/snippets/snippet-with-region.js" region="snippet" lines="1,2" lang="ts" attrs=":line-numbers" title="snippet with region" />
