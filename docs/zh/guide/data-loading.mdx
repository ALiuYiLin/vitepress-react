...
然后，可以在 `.mdx` 页面（或 `.tsx` 组件）中通过 `data` 具名导出从该文件导入数据：直接把它作为模块顶层 import（可被正文 `{expr}` 引用）：
...
默认情况下只提供 `url` 和 `frontmatter`。这是因为加载的数据将作为 JSON 内联在客户端 bundle 中，我们需要谨慎考虑其大小。下面的例子展示了如何使用数据构建最小的博客索引页面（多行 JSX 与表达式在 mdx 中直接写在正文即可，见[在 Markdown 中使用 React](./using-react)）：
