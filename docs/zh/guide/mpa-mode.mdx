...
> 注：本页示例描述的是旧 markdown-it 内核（.md 页）的能力。当前 mdx 内核下页面脚本机制（含 `<script client>` 的 MPA 独立打包）待迁移，需要少量客户端交互的页面暂时可改用顶层 `import` 引入脚本模块（会随页面 bundle 加载）；MPA 独立脚本能力将在内核支持后补充示例。
