...
- 已迁移组件(文件已存在于 `src/client/theme-default/components/`):
  - 基础:`VPBackdrop`、`VPBadge`、`VPIcon`、`VPImage`、`VPLink`、`VPSkipLink`(早前子代理落盘)、`VPButton`(自己迁)、`VPPage`、`VPSwitch`、`VPSocialLink`、`VPMenuLink`(自己迁)、`VPFooter`、`VPSwitchAppearance`、`VPMenuGroup`、`VPMenu`、`VPDocOutlineItem`、`VPDocAsideOutline`、`VPDocAside`、`VPDocFooter`、`VPDoc`(自己迁)。
