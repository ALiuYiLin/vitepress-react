...
2. **第一版(v1)不做"作者免转义"适配层**(2026-09-06 拍板):作者手动写 `\{…\}`。自动转义方案(后缀位置 + acorn 判别,防误转义行尾真表达式)已评估完,记入 **backlog(后续优化方向)**:核心切换稳定后再评估是否引入,届时以"位置状态机复用 maskJsxExpressions + 内容判别 + 防双重转义"为设计基线。
