...
export interface MarkdownCompileResult {
  /**
   * 生成的 React 页面模块源码(TSX 文本;由 plugin.ts 内的 esbuild
   * 以 automatic JSX runtime 编译成可执行 JS)。
   *
   * M0:整页 markdown HTML 经 dangerouslySetInnerHTML 注入(与 SSR HTML
   * 两端一致,保证水合可用)。M1 会替换为 HTML→JSX 序列化 + script 块
   * 模块顶层提升 + 组件引用解析(对应迁移 D1/D2)。
   */
  reactSrc: string
  pageData: PageData
...
export async function createMarkdownToReactRenderFn(
  ...
  return async (src: string, file: string): Promise<MarkdownCompileResult> => {
    ...
    // M0:整页 HTML 交给 React 的 dangerouslySetInnerHTML(见模块头注释);
    // M1 起改为 HTML→JSX 序列化并支持 <script> 块(正文双花括号一律字面,D1)。
    const reactSrc = [
      '// generated by vitepress (React runtime)',
      `export const __pageData = JSON.parse(${JSON.stringify(
        JSON.stringify(pageData)
      )})`,
      'export default function Page() {',
      `  return <div className="vp-doc" dangerouslySetInnerHTML={{ __html: ${JSON.stringify(
        html
      )} }} />`,
      '}'
    ].join('\n')
