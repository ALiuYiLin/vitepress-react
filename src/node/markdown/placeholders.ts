...
// 由 markdownToReact.ts 重构拆分而来:
//   - 写入方:markdown/jsxMasking.ts(maskScriptBlocks / maskJsxHtmlLines /
//     maskJsxExpressions);
//   - 读取方:markdownToReact.ts 编排层(restoreHeaderExpressions)、
//     markdown/serializeHtmlToJsx.ts(textWithExpr / data-vp-jsx 哨兵还原)。
// 任何一侧调整 token 格式都必须同步到这里,保证「写入格式 == 读取格式」。

/** mask 阶段每一条占位记录:expr / html 二选一(store 数组下标即占位编号) */
export interface PlaceholderEntry {
  /** 正文 JSX 表达式原码(还原为 `{code}`,与 Page 作用域共享) */
  expr?: string
  /** 作者写的 JSX 标签原文(整行/整块占位,不进入 markdown-it) */
  html?: string
}

/** maskScriptBlocks / maskJsxHtmlLines / maskJsxExpressions 共用的记录数组 */
...
/** 正文 JSX 表达式占位 token(maskJsxExpressions 写入) */
export const exprToken = (n: number) => `@@VP_EXPR_${n}@@`

/** 行内 JSX 标签占位 token(maskJsxHtmlLines 写入,序列化时原样还原) */
export const htmlToken = (n: number) => `@@VP_HTML_${n}@@`

/** 占位 token 前缀(快速短路判断用,无需构造完整正则) */
export const VP_TOKEN_MARKER = '@@VP_'

/** EXPR/HTML 占位全局正则(带 /g;调用方负责 lastIndex 复位或交给 replace) */
export const VP_TOKEN_GLOBAL_RE = /@@VP_(EXPR|HTML)_(\d+)@@/g
